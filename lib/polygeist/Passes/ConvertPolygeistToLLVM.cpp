...
static void emitHipError(const llvm::Twine &expr, const char *buffer, hipError_t result, Location loc) {
    const char *error;
    error = hipGetErrorString(result);
    emitError(
        loc, expr.concat(" failed with error code ").concat(llvm::Twine{error}).concat("[").concat(buffer).concat("]"));
}
...
static void emitCudaError(const llvm::Twine &expr, const char *buffer, CUresult result, Location loc) {
    const char *error;
...
Type convertMemrefElementTypeForLLVMPointer(MemRefType type, const LLVMTypeConverter &converter) {
...
struct UndefLowering : public ConvertOpToLLVMPattern<UndefOp> {
    ...
    LogicalResult matchAndRewrite(UndefOp uop, OpAdaptor adaptor, ConversionPatternRewriter &rewriter) const override {
...
struct SubIndexOpLowering : public ConvertOpToLLVMPattern<SubIndexOp> {
    ...
    LogicalResult matchAndRewrite(SubIndexOp subViewOp, OpAdaptor transformed,
        ...
        if (transformed.getSource().getType().isa<LLVM::LLVMPointerType>()) {
            ...
            auto elTy = convertMemrefElementTypeForLLVMPointer(subViewOp.getSource().getType(), *getTypeConverter());
            if (viewMemRefType.getShape().size() != sourceMemRefType.getShape().size()) {
            ...
            auto ptr = rewriter.create<LLVM::GEPOp>(loc, t, elTy, transformed.getSource(), indices);
            std::vector ptrs = {ptr.getResult()};
            rewriter.replaceOpWithNewOp<LLVM::BitcastOp>(subViewOp,
                                                         getTypeConverter()->convertType(subViewOp.getType()), ptrs);
            return success();
        }
        ...
        if (sourceMemRefType.getShape().size() != viewMemRefType.getShape().size()) {
            ...
            Value cop = rewriter.create<LLVM::ConstantOp>(loc, idxs[0].getType(),
        ...
        MemRefDescriptor nexRef = createMemRefDescriptor(
            ...
            rewriter.create<LLVM::GEPOp>(loc, prev.getType(), prev, idxs), sizes, strides, rewriter);
...
struct Memref2PointerOpLowering : public ConvertOpToLLVMPattern<Memref2PointerOp> {
    ...
    LogicalResult matchAndRewrite(Memref2PointerOp op, OpAdaptor transformed,
                                  ConversionPatternRewriter &rewriter) const override {
        auto loc = op.getLoc();
        ...
        if (transformed.getSource().getType().isa<LLVM::LLVMPointerType>()) {
            mlir::Value ptr = rewriter.create<LLVM::BitcastOp>(loc, LLVM::LLVMPointerType::get(op.getContext(), space0),
        ...
        MemRefDescriptor targetMemRef(transformed.getSource()); // MemRefDescriptor::undef(rewriter, loc,
                                                                // targetDescTy);
        ...
        ptr = rewriter.create<LLVM::BitcastOp>(loc, LLVM::LLVMPointerType::get(op.getContext(), space0), ptr);
        if (space0 != LPT.getAddressSpace())
            ptr = rewriter.create<LLVM::AddrSpaceCastOp>(loc, LPT, ptr);
...
struct Pointer2MemrefOpLowering : public ConvertOpToLLVMPattern<Pointer2MemrefOp> {
    using ConvertOpToLLVMPattern<Pointer2MemrefOp>::ConvertOpToLLVMPattern;

    LogicalResult matchAndRewrite(Pointer2MemrefOp op, OpAdaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const override {
        auto loc = op.getLoc();
        ...
        auto ptr = rewriter.create<LLVM::BitcastOp>(op.getLoc(), descr.getElementPtrType(), adaptor.getSource());
...
struct StreamToTokenOpLowering : public ConvertOpToLLVMPattern<StreamToTokenOp> {
    using ConvertOpToLLVMPattern<StreamToTokenOp>::ConvertOpToLLVMPattern;

    LogicalResult matchAndRewrite(StreamToTokenOp op, OpAdaptor transformed,
                                  ConversionPatternRewriter &rewriter) const override {
...
struct TypeSizeOpLowering : public ConvertOpToLLVMPattern<TypeSizeOp> {
    ...
    LogicalResult matchAndRewrite(TypeSizeOp op, OpAdaptor transformed,
                                  ConversionPatternRewriter &rewriter) const override {
        ...
        if (NT.isa<IntegerType, FloatType>() || LLVM::isCompatibleType(NT)) {
            ...
            rewriter.replaceOpWithNewOp<LLVM::ConstantOp>(op, type, rewriter.getIntegerAttr(type, DLI.getTypeSize(NT)));
            return success();
        }
...
struct TypeAlignOpLowering : public ConvertOpToLLVMPattern<TypeAlignOp> {
    ...
    LogicalResult matchAndRewrite(TypeAlignOp op, OpAdaptor transformed,
                                  ConversionPatternRewriter &rewriter) const override {
        ...
        if (NT.isa<IntegerType, FloatType>() || LLVM::isCompatibleType(NT)) {
            ...
            rewriter.replaceOpWithNewOp<LLVM::ConstantOp>(op, type,
                                                          rewriter.getIntegerAttr(type, DLI.getTypeABIAlignment(NT)));
            return success();
        }
...
struct GPUGlobalConversion : public OpRewritePattern<memref::GlobalOp> {
    ...
    LogicalResult matchAndRewrite(memref::GlobalOp globalOp, PatternRewriter &rewriter) const override {
        ...
        auto type = MemRefType::get(mt.getShape(), mt.getElementType(), {}, newMemspace);
        ...
        auto newGlobalOp =
            rewriter.create<memref::GlobalOp>(globalOp->getLoc(), rewriter.getStringAttr(globalOp.getSymName()),
...
struct GPUGetGlobalConversion : public OpRewritePattern<memref::GetGlobalOp> {
    ...
    LogicalResult matchAndRewrite(memref::GetGlobalOp ggo, PatternRewriter &rewriter) const override {
        ...
        auto globalOp = cast<memref::GlobalOp>(gpuModule.lookupSymbol(ggo.getNameAttr()));
        int newMemspace = 0;
        int globalMemspace = globalOp.getType().getMemorySpaceAsInt();
        if (globalOp->getAttr("polygeist.cuda_device") || globalMemspace == 1) {
            newMemspace = 1;
        } else if (globalOp->getAttr("polygeist.cuda_constant") || globalMemspace == 4) {
            newMemspace = 4;
        } else {
            // TODO what else is there? managed?
        ...
        auto newMT = MemRefType::get(mt.getShape(), mt.getElementType(), {}, newMemspace);
...
struct LLVMOpLowering : public ConversionPattern {
    explicit LLVMOpLowering(LLVMTypeConverter &converter)
        : ConversionPattern(converter, Pattern::MatchAnyOpTypeTag(), 1, &converter.getContext()) {}
    ...
    LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
        ...
        if (failed(converter->convertTypes(op->getResultTypes(), convertedResultTypes))) {
            return failure();
        }
        SmallVector<Type> convertedOperandTypes;
        if (failed(converter->convertTypes(op->getOperandTypes(), convertedOperandTypes))) {
            return failure();
        }
        ...
        if (convertedResultTypes == op->getResultTypes() && convertedOperandTypes == op->getOperandTypes() &&
            typeAttrsConverted) {
            return failure();
        }
        ...
        for (unsigned i = 0, e = op->getNumRegions(); i < e; ++i)
            rewriter.inlineRegionBefore(op->getRegion(i), rewritten->getRegion(i), rewritten->getRegion(i).begin());
...
struct URLLVMOpLowering : public ConvertOpToLLVMPattern<UnrealizedConversionCastOp> {
...
static LLVM::LLVMFuncOp addMocCUDAFunction(ModuleOp module, Type streamTy) {
    ...
    auto resumeOp =
        moduleBuilder.create<LLVM::LLVMFuncOp>(fname, LLVM::LLVMFunctionType::get(voidTy, {ptrTy, ptrTy, streamTy}));
    resumeOp.setPrivate();
...
class ConvertExecuteOpTypes : public ConvertOpToLLVMPattern<async::ExecuteOp> {
  public:
    ...
    LogicalResult matchAndRewrite(async::ExecuteOp op, OpAdaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const override {
        async::ExecuteOp newOp = cast<async::ExecuteOp>(rewriter.cloneWithoutRegions(*op.getOperation()));
...
struct AsyncOpLowering : public ConvertOpToLLVMPattern<async::ExecuteOp> {
    ...
    LogicalResult matchAndRewrite(async::ExecuteOp execute, OpAdaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const override {
        ...
        const TypeConverter *converter = getTypeConverter();
        auto typesRange =
            llvm::map_range(functionInputs, [&](Value value) { return converter->convertType(value.getType()); });
        ...
        LLVM::LLVMFuncOp func;
        {
            ...
            func = rewriter.create<LLVM::LLVMFuncOp>(
                execute.getLoc(), "kernelbody." + std::to_string((long long int)&execute) + "." + std::to_string(off),
                funcType);
        }
        ...
        for (Value capture : toErase) {
            ...
            for (auto r : llvm::zip(op->getResults(), rewriter.clone(*op, valueMapping)->getResults())) {
                valueMapping.map(rewriter.getRemappedValue(std::get<0>(r)), std::get<1>(r));
            }
        }
        // Prepare for coroutine conversion by creating the body of the function.
        {
            ...
            if (functionInputs.size() == 0) {
                       ...
                       converter->convertType(functionInputs[0].getType()).isa<LLVM::LLVMPointerType>()) {
                valueMapping.map(functionInputs[0],
                                 rewriter.create<LLVM::BitcastOp>(
                                     execute.getLoc(), converter->convertType(functionInputs[0].getType()), arg));
            } else if (functionInputs.size() == 1 &&
                       converter->convertType(functionInputs[0].getType()).isa<IntegerType>()) {
                valueMapping.map(functionInputs[0],
                ...
                auto alloc = rewriter.create<LLVM::BitcastOp>(execute.getLoc(), LLVM::LLVMPointerType::get(ctx), arg);
                for (auto idx : llvm::enumerate(functionInputs)) {
                    ...
                    Value next = rewriter.create<LLVM::GEPOp>(loc, LLVM::LLVMPointerType::get(ctx),
                                                              idx.value().getType(), alloc, idxs);
                    valueMapping.map(idx.value(), rewriter.create<LLVM::LoadOp>(loc, idx.value().getType(), next));
                }
        ...
        // Replace the original `async.execute` with a call to outlined function.
        {
            ...
            if (crossing.size() == 0) {
                vals.push_back(rewriter.create<LLVM::ZeroOp>(execute.getLoc(), voidPtr));
            } else if (crossing.size() == 1 &&
                       converter->convertType(crossing[0].getType()).isa<LLVM::LLVMPointerType>()) {
                vals.push_back(rewriter.create<LLVM::BitcastOp>(execute.getLoc(), voidPtr, crossing[0]));
            } else if (crossing.size() == 1 && converter->convertType(crossing[0].getType()).isa<IntegerType>()) {
                vals.push_back(rewriter.create<LLVM::IntToPtrOp>(execute.getLoc(), voidPtr, crossing[0]));
            } else {
                SmallVector<Type> types;
                for (auto v : crossing)
                ...
                Value arg = rewriter.create<arith::IndexCastOp>(
                    ...
                    rewriter.create<polygeist::TypeSizeOp>(loc, rewriter.getIndexType(), ST));
                auto mallocFunc = LLVM::lookupOrCreateMallocFn(module, getIndexType(),
                                                               /*opaquePointers=*/true);
                mlir::Value alloc = rewriter.create<LLVM::CallOp>(loc, mallocFunc, arg).getResult();
                rewriter.setInsertionPoint(execute);
                for (auto idx : llvm::enumerate(crossing)) {
                    ...
                    Value next = rewriter.create<LLVM::GEPOp>(loc, LLVM::LLVMPointerType::get(rewriter.getContext()),
                                                              idx.value().getType(), alloc, idxs);
                    rewriter.create<LLVM::StoreOp>(loc, idx.value(), next);
                }
                vals.push_back(rewriter.create<LLVM::BitcastOp>(execute.getLoc(), voidPtr, alloc));
            }
            vals.push_back(rewriter.create<LLVM::BitcastOp>(
                execute.getLoc(), voidPtr, rewriter.create<LLVM::AddressOfOp>(execute.getLoc(), func)));
            for (auto dep : execute.getDependencies()) {
                ...
                if (auto MT = dyn_cast<MemRefType>(src.getType()))
                    src = rewriter.create<polygeist::Memref2PointerOp>(
                        ...
                        LLVM::LLVMPointerType::get(rewriter.getContext(), MT.getMemorySpaceAsInt()), src);
                vals.push_back(src);
            }
            assert(vals.size() == 3);

            auto f = addMocCUDAFunction(execute->getParentOfType<ModuleOp>(), vals.back().getType());
...
struct GlobalOpTypeConversion : public OpConversionPattern<LLVM::GlobalOp> {
    explicit GlobalOpTypeConversion(LLVMTypeConverter &converter)
        : OpConversionPattern<LLVM::GlobalOp>(converter, &converter.getContext()) {}

    LogicalResult matchAndRewrite(LLVM::GlobalOp op, LLVM::GlobalOp::Adaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const override {
        const TypeConverter *converter = getTypeConverter();
        ...
        rewriter.updateRootInPlace(op, [&]() { op.setGlobalTypeAttr(TypeAttr::get(convertedType)); });
        return success();
    }
};
...
struct GetFuncOpConversion : public OpConversionPattern<polygeist::GetFuncOp> {
    explicit GetFuncOpConversion(LLVMTypeConverter &converter)
        : OpConversionPattern<polygeist::GetFuncOp>(converter, &converter.getContext()) {}

    LogicalResult matchAndRewrite(polygeist::GetFuncOp op, polygeist::GetFuncOp::Adaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const override {
        const TypeConverter *converter = getTypeConverter();
        ...
        rewriter.replaceOpWithNewOp<LLVM::AddressOfOp>(op, convertedType, op.getName());
...
struct ReturnOpTypeConversion : public ConvertOpToLLVMPattern<LLVM::ReturnOp> {
    ...
    LogicalResult matchAndRewrite(LLVM::ReturnOp op, LLVM::ReturnOp::Adaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const override {
        auto replacement = rewriter.replaceOpWithNewOp<LLVM::ReturnOp>(op, adaptor.getArg());
...
template <typename OpTy> struct AllocLikeOpLowering : public ConvertOpToLLVMPattern<OpTy> {
  public:
  ...
  protected:
    ...
    Value getOuterSize(OpTy original, typename ConvertOpToLLVMPattern<OpTy>::OpAdaptor adaptor,
        ...
        return this->createIndexAttrConstant(rewriter, original->getLoc(), indexType,
                                             original.getType().getRank() == 0 ? 1 : original.getType().getDimSize(0));
    }
};
...
struct CAllocaOpLowering : public AllocLikeOpLowering<memref::AllocaOp> {
  public:
    ...
    LogicalResult matchAndRewrite(memref::AllocaOp allocaOp, OpAdaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const override {
        Location loc = allocaOp.getLoc();
        MemRefType originalType = allocaOp.getType();
        auto convertedType = dyn_cast_or_null<LLVM::LLVMPointerType>(getTypeConverter()->convertType(originalType));
        auto elTy = convertMemrefElementTypeForLLVMPointer(originalType, *this->getTypeConverter());
        if (!convertedType || !elTy)
            return rewriter.notifyMatchFailure(loc, "unsupported memref type");

        assert(adaptor.getDynamicSizes().size() <= 1 && "expected at most one dynamic size");

        Value outerSize = getOuterSize(allocaOp, adaptor, rewriter);
        rewriter.replaceOpWithNewOp<LLVM::AllocaOp>(allocaOp, convertedType, elTy, outerSize,
...
struct CAllocOpLowering : public AllocLikeOpLowering<memref::AllocOp> {
  public:
    ...
    LogicalResult matchAndRewrite(memref::AllocOp allocOp, OpAdaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const override {
        ...
        Value elementSize = rewriter.create<polygeist::TypeSizeOp>(loc, rewriter.getIndexType(),
        ...
        if (auto F = module.lookupSymbol<mlir::func::FuncOp>("malloc")) {
            Value allocated = rewriter.create<func::CallOp>(loc, F, size).getResult(0);
            rewriter.replaceOpWithNewOp<polygeist::Memref2PointerOp>(allocOp, convertedType, allocated);
        } else {
            LLVM::LLVMFuncOp mallocFunc = getTypeConverter()->getOptions().useGenericFunctions
                                              ? LLVM::lookupOrCreateGenericAllocFn(module, getIndexType(),
                                                                                   /*opaquePointers=*/true)
                                              : LLVM::lookupOrCreateMallocFn(module, getIndexType(),
                                                                             /*opaquePointers=*/true);
            Value allocated = rewriter.create<LLVM::CallOp>(loc, mallocFunc, size).getResult();
            rewriter.replaceOpWithNewOp<LLVM::BitcastOp>(allocOp, convertedType, allocated);
        }
        return success();
    }
};
...
struct CDeallocOpLowering : public ConvertOpToLLVMPattern<memref::DeallocOp> {
  public:
    ...
    LogicalResult matchAndRewrite(memref::DeallocOp deallocOp, OpAdaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const override {
        ...
        if (auto F = module.lookupSymbol<mlir::func::FuncOp>("free")) {
            Value casted = rewriter.create<polygeist::Pointer2MemrefOp>(
                deallocOp->getLoc(), MemRefType::get({-1}, rewriter.getI8Type()), adaptor.getMemref());
            rewriter.replaceOpWithNewOp<func::CallOp>(deallocOp, F, casted);
        } else {
            LLVM::LLVMFuncOp freeFunc = getTypeConverter()->getOptions().useGenericFunctions
                                            ? LLVM::lookupOrCreateGenericFreeFn(module,
                                                                                /*opaquePointers*/ true)
                                            : LLVM::lookupOrCreateFreeFn(module, /*opaquePointers*/ true);
...
static Type convertGlobalMemRefTypeToLLVM(MemRefType type, const TypeConverter &typeConverter) {
...
struct GlobalOpLowering : public ConvertOpToLLVMPattern<memref::GlobalOp> {
  public:
    ...
    LogicalResult matchAndRewrite(memref::GlobalOp globalOp, OpAdaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const override {
        MemRefType originalType = globalOp.getType();
        if (!originalType.hasStaticShape() || !originalType.getLayout().isIdentity()) {
            return rewriter.notifyMatchFailure(globalOp->getLoc(), "unsupported type");
        }

        Type convertedType = convertGlobalMemRefTypeToLLVM(originalType, *typeConverter);
        LLVM::Linkage linkage = globalOp.isPublic() ? LLVM::Linkage::External : LLVM::Linkage::Private;
        if (!convertedType) {
            return rewriter.notifyMatchFailure(globalOp->getLoc(), "failed to convert memref type");
        }
        ...
        bool dso_local = globalOp->getAttr("polygeist.cuda_device") || globalOp->getAttr("polygeist.cuda_constant");
        bool thread_local_ = false;
        LLVM::UnnamedAddrAttr unnamed_addr = nullptr;
        StringAttr section = nullptr;
        auto newGlobal = rewriter.replaceOpWithNewOp<LLVM::GlobalOp>(
            globalOp, convertedType, globalOp.getConstant(), globalOp.getSymName(), linkage, dso_local, thread_local_,
            initialValue, alignment, originalType.getMemorySpaceAsInt(), unnamed_addr, section,
            /*comdat=*/nullptr);
        if (!globalOp.isExternal() && globalOp.isUninitialized()) {
            Block *block =
                rewriter.createBlock(&newGlobal.getInitializerRegion(), newGlobal.getInitializerRegion().begin());
            rewriter.setInsertionPointToStart(block);
            Value undef = rewriter.create<LLVM::UndefOp>(globalOp->getLoc(), convertedType);
...
struct GetGlobalOpLowering : public ConvertOpToLLVMPattern<memref::GetGlobalOp> {
  public:
    using ConvertOpToLLVMPattern<memref::GetGlobalOp>::ConvertOpToLLVMPattern;

    LogicalResult matchAndRewrite(memref::GetGlobalOp getGlobalOp, OpAdaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const override {
        ...
        Value wholeAddress =
            rewriter.create<LLVM::AddressOfOp>(getGlobalOp->getLoc(), convertedType, getGlobalOp.getName());
...
template <typename OpTy> struct CLoadStoreOpLowering : public ConvertOpToLLVMPattern<OpTy> {
  protected:
    using ConvertOpToLLVMPattern<OpTy>::ConvertOpToLLVMPattern;

    /// Emits the IR that computes the address of the memory being accessed.
    Value getAddress(OpTy op, typename ConvertOpToLLVMPattern<OpTy>::OpAdaptor adaptor,
                     ConversionPatternRewriter &rewriter) const {
        Location loc = op.getLoc();
        MemRefType originalType = op.getMemRefType();
        auto convertedType =
            dyn_cast_or_null<LLVM::LLVMPointerType>(this->getTypeConverter()->convertType(originalType));
        if (!convertedType) {
        ...
        SmallVector<LLVM::GEPArg> args =
            llvm::to_vector(llvm::map_range(adaptor.getIndices(), [](Value v) { return LLVM::GEPArg(v); }));
        auto elTy = convertMemrefElementTypeForLLVMPointer(originalType, *this->getTypeConverter());
        if (!elTy) {
            (void)rewriter.notifyMatchFailure(loc, "unsupported memref type");
            return nullptr;
        ...
        return rewriter.create<LLVM::GEPOp>(
            loc, LLVM::LLVMPointerType::get(op.getContext(), originalType.getMemorySpaceAsInt()), elTy,
...
struct CLoadOpLowering : public CLoadStoreOpLowering<memref::LoadOp> {
  public:
    ...
    LogicalResult matchAndRewrite(memref::LoadOp loadOp, OpAdaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const override {
        ...
        rewriter.replaceOpWithNewOp<LLVM::LoadOp>(
            loadOp, typeConverter->convertType(loadOp.getMemRefType().getElementType()), address);
        return success();
    }
};
...
struct CAtomicRMWOpLowering : public CLoadStoreOpLowering<memref::AtomicRMWOp> {
    ...
    LogicalResult matchAndRewrite(memref::AtomicRMWOp atomicOp, OpAdaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const override {
        ...
        if (!dataPtr)
            return failure();
        rewriter.replaceOpWithNewOp<LLVM::AtomicRMWOp>(atomicOp, *maybeKind, dataPtr, adaptor.getValue(),
...
struct CStoreOpLowering : public CLoadStoreOpLowering<memref::StoreOp> {
  public:
    ...
    LogicalResult matchAndRewrite(memref::StoreOp storeOp, OpAdaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const override {
        Value address = getAddress(storeOp, adaptor, rewriter);
        if (!address)
            return failure();

        rewriter.replaceOpWithNewOp<LLVM::StoreOp>(storeOp, adaptor.getValue(), address);
        return success();
    }
};
} // namespace
...
static void filterFuncAttributes(func::FuncOp func, bool filterArgAndResAttrs,
    ...
    for (const NamedAttribute &attr : func->getAttrs()) {
        if (attr.getName() == SymbolTable::getSymbolAttrName() || attr.getName() == func.getFunctionTypeAttrName() ||
...
static SmallVector<NamedAttribute> convertFuncAttributes(func::FuncOp funcOp, const TypeConverter &typeConverter,
    ...
    if (ArrayAttr resAttrDicts = funcOp.getAllResultAttrs()) {
        ...
        auto newResAttrDicts =
            (funcOp.getNumResults() == 1) ? resAttrDicts : rewriter.getArrayAttr(rewriter.getDictionaryAttr({}));
        attributes.push_back(rewriter.getNamedAttr(funcOp.getResAttrsAttrName(), newResAttrDicts));
    }
        ...
        for (unsigned i = 0, e = funcOp.getNumArguments(); i < e; ++i) {
            ...
            for (const NamedAttribute &attr : attrsDict) {
                const auto convert = [&](const NamedAttribute &attr) {
                    return TypeAttr::get(typeConverter.convertType(attr.getValue().cast<TypeAttr>().getValue()));
                };
                if (attr.getName().getValue() == LLVM::LLVMDialect::getByValAttrName()) {
                    convertedAttrs.push_back(
                        rewriter.getNamedAttr(LLVM::LLVMDialect::getByValAttrName(), convert(attr)));
                } else if (attr.getName().getValue() == LLVM::LLVMDialect::getByRefAttrName()) {
                    convertedAttrs.push_back(
                        rewriter.getNamedAttr(LLVM::LLVMDialect::getByRefAttrName(), convert(attr)));
                } else if (attr.getName().getValue() == LLVM::LLVMDialect::getStructRetAttrName()) {
                    convertedAttrs.push_back(
                        rewriter.getNamedAttr(LLVM::LLVMDialect::getStructRetAttrName(), convert(attr)));
                } else if (attr.getName().getValue() == LLVM::LLVMDialect::getInAllocaAttrName()) {
                    convertedAttrs.push_back(
                        rewriter.getNamedAttr(LLVM::LLVMDialect::getInAllocaAttrName(), convert(attr)));
                } else {
            ...
            for (size_t j = 0; j < mapping->size; ++j)
                newArgAttrs[mapping->inputNo + j] = DictionaryAttr::get(rewriter.getContext(), convertedAttrs);
        }
        attributes.push_back(rewriter.getNamedAttr(funcOp.getArgAttrsAttrName(), rewriter.getArrayAttr(newArgAttrs)));
    }
...
static Type convertAndPackFunctionResultType(FunctionType type, const TypeConverter &typeConverter) {
    SmallVector<Type> convertedResultTypes;
    if (failed(typeConverter.convertTypes(type.getResults(), convertedResultTypes)))
        return nullptr;
    ...
    return LLVM::LLVMStructType::getLiteral(type.getContext(), convertedResultTypes);
}
...
static std::optional<std::pair<LLVM::LLVMFunctionType, TypeConverter::SignatureConversion>>
convertFunctionType(FuncOpType funcOp, const TypeConverter &typeConverter) {
    ...
    Type resultType = convertAndPackFunctionResultType(funcOp.getFunctionType(), typeConverter);
    if (!resultType)
        return std::nullopt;

    auto varargsAttr = funcOp->template getAttrOfType<BoolAttr>("func.varargs");
    auto convertedType = LLVM::LLVMFunctionType::get(resultType, signatureConversion.getConvertedTypes(),
...
class ConvertLaunchFuncOpToGpuRuntimeCallPattern : public ConvertOpToGpuRuntimeCallPattern<gpu::LaunchFuncOp> {
  public:
    ConvertLaunchFuncOpToGpuRuntimeCallPattern(LLVMTypeConverter &typeConverter, StringRef gpuBinaryAnnotation,
                                               std::string gpuTarget, bool mgpuDebugLaunches = false)
        : ConvertOpToGpuRuntimeCallPattern<gpu::LaunchFuncOp>(typeConverter), gpuBinaryAnnotation(gpuBinaryAnnotation),
          gpuTarget(gpuTarget), mgpuDebugLaunches(mgpuDebugLaunches) {}

  private:
    Value generateParamsArray(gpu::LaunchFuncOp launchOp, OpAdaptor adaptor, OpBuilder &builder) const;
    Value generateKernelNameConstant(StringRef moduleName, StringRef name, Location loc, OpBuilder &builder) const;

    LogicalResult matchAndRewrite(gpu::LaunchFuncOp launchOp, OpAdaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const override;
...
template <typename Stream, class Tuple> struct TuplePrinter<Stream, Tuple, 1> {
    static void print(Stream &stream, const Tuple &t) { stream << std::get<0>(t); }
};
template <typename Stream, typename... Args, std::enable_if_t<sizeof...(Args) != 0, int> = 0>
void print(Stream &stream, const std::tuple<Args...> &t) {
    TuplePrinter<Stream, decltype(t), sizeof...(Args)>::print(stream, t);
}

struct LowerGPUAlternativesOp : public OpRewritePattern<polygeist::AlternativesOp>, public GpuRuntimeCallBuilders {
    using OpRewritePattern<polygeist::AlternativesOp>::OpRewritePattern;
    const char *PATTERN = "lower-gpu-alternatives";

    LogicalResult matchAndRewrite(polygeist::AlternativesOp gao, PatternRewriter &rewriter) const override {

        if (gao->getAttrOfType<StringAttr>("alternatives.type").getValue() != "gpu_kernel")
            return failure();
        ...
        typedef std::tuple<Region *, int, int, int, int, int, int, int, int, int, int, int, int> kernelInfoTy;
        ...
        auto gatherInfos = [&]() {
            ...
            auto gatherLLVMInfos = [&](Operation *gpuFunc) -> kernelLLVMInfoTy {
                int ops = 0, floatOps = 0, intOps = 0, loads = 0, stores = 0, branches = 0;
                ...
                gpuFunc->walk([&](Operation *op) {
                    ...
                    if (isa<LLVM::BrOp>(op)) {
                        branches++;
                    } else if (isa<LLVM::FAddOp>(op) || isa<LLVM::FMulOp>(op) || isa<LLVM::FDivOp>(op) ||
                               isa<LLVM::FSubOp>(op) || isa<LLVM::FRemOp>(op)) {
                        int width = dyn_cast<FloatType>(op->getOperand(0).getType()).getWidth();
                    ...
                    } else if (isa<LLVM::AddOp>(op) || isa<LLVM::SubOp>(op) || isa<LLVM::MulOp>(op) ||
                               isa<LLVM::UDivOp>(op) || isa<LLVM::SDivOp>(op)) {
                        intOps++;
            ...
            if (gpuTarget == "cuda") {
                ...
                for (auto &region : gao->getRegions()) {
                    ...
                    RETURN_ON_CUDA_ERROR(
                        cuFuncGetAttribute(&maxThreadsPerBlock, CU_FUNC_ATTRIBUTE_MAX_THREADS_PER_BLOCK, cuFunction));
                    RETURN_ON_CUDA_ERROR(
                        cuFuncGetAttribute(&sharedMemSize, CU_FUNC_ATTRIBUTE_SHARED_SIZE_BYTES, cuFunction));
                    RETURN_ON_CUDA_ERROR(
                        cuFuncGetAttribute(&constMemSize, CU_FUNC_ATTRIBUTE_CONST_SIZE_BYTES, cuFunction));
                    RETURN_ON_CUDA_ERROR(
                        cuFuncGetAttribute(&localMemSize, CU_FUNC_ATTRIBUTE_LOCAL_SIZE_BYTES, cuFunction));
                    ...
                    for (auto dim : {blockDims.x, blockDims.y, blockDims.z}) {
                        ...
                        } else if (auto cstindex = dyn_cast_or_null<arith::ConstantIndexOp>(dim.getDefiningOp())) {
                    ...
                    infos.push_back(std::tuple_cat(std::make_tuple(&region),
                                                   std::make_tuple(localMemSize, occupancyNumBlocks, numRegs, blockSize,
                                                                   sharedMemSize, constMemSize),
                                                   kernelLLVMInfo));
                }
            }
#endif
            ...
            if (gpuTarget == "rocm") {
                ...
                for (auto &region : gao->getRegions()) {
                    ...
                    RETURN_ON_HIP_ERROR(hipModuleGetFunction(&hipFunction, hipModule, launchOp.getKernelName().data()));
                    ...
                    // here but ptxas outputs it? should we parse the ptxas output and add
                    // an attribute for those values
                    RETURN_ON_HIP_ERROR(hipFuncGetAttribute(&maxThreadsPerBlock,
                                                            HIP_FUNC_ATTRIBUTE_MAX_THREADS_PER_BLOCK, hipFunction));
                    RETURN_ON_HIP_ERROR(
                        hipFuncGetAttribute(&sharedMemSize, HIP_FUNC_ATTRIBUTE_SHARED_SIZE_BYTES, hipFunction));
                    RETURN_ON_HIP_ERROR(
                        hipFuncGetAttribute(&constMemSize, HIP_FUNC_ATTRIBUTE_CONST_SIZE_BYTES, hipFunction));
                    RETURN_ON_HIP_ERROR(
                        hipFuncGetAttribute(&localMemSize, HIP_FUNC_ATTRIBUTE_LOCAL_SIZE_BYTES, hipFunction));
                    RETURN_ON_HIP_ERROR(hipFuncGetAttribute(&numRegs, HIP_FUNC_ATTRIBUTE_NUM_REGS, hipFunction));
                    ...
                    if (blockSize > 0) {
                        auto succeeded =
                            [&]() {
                                RETURN_ON_HIP_ERROR(hipOccupancyMaxActiveBlocksPerMultiprocessor(
                                    &occupancyNumBlocks, hipFunction, blockSize, dynamicSharedMemSize));
                                return success();
                            }()
                                .succeeded();
                        ...
                        if (!succeeded) {
                            llvm::errs() << "Why does this fail with block size " << blockSize
                                         << " and dynamic shared mem size " << dynamicSharedMemSize << " \n";
                            occupancyNumBlocks = 0;
                        }
                    } else {
                        occupancyNumBlocks = 0;
        ...
        auto sortInfos = [&]() {
            auto getCost = [](auto a) -> double {
                ...
                return coefficients[0] * std::get<0>(a) + coefficients[1] * std::get<1>(a) +
                       coefficients[2] * std::get<2>(a) + coefficients[3] * std::get<3>(a) + 0 * std::get<4>(a) +
                       0 * std::get<5>(a);
            };
        ...
        if (PolygeistAlternativesMode == PAM_Static) {
            ...
            auto kernelId = LLVM::createGlobalString(loc, rewriter, std::string("kernelId.") + std::to_string(num++),
                                                     nullTermLocStr, LLVM::Linkage::Internal, /*opaquePointers*/ true);
            auto totalAlternatives = rewriter.create<LLVM::ConstantOp>(loc, llvmInt32Type, gao->getNumRegions());
            auto alternative =
                rtPGOGetAlternativeCallBuilder.create(loc, rewriter, {kernelId, totalAlternatives})->getResult(0);

            int i = 0;
            for (auto &region : gao->getRegions()) {
                auto cmpOp = rewriter.create<arith::CmpIOp>(loc, arith::CmpIPredicate::eq, alternative,
                                                            rewriter.create<arith::ConstantIntOp>(loc, i, 32));
                auto ifOp = rewriter.create<scf::IfOp>(loc, cmpOp, /* hasElse */ true);
                auto block = &region.front();
                rewriter.eraseOp(block->getTerminator());
                rewriter.inlineBlockBefore(block, ifOp.getThenRegion().front().getTerminator());

                // Timing
                rewriter.setInsertionPointToStart(&ifOp.getThenRegion().front());
                rtPGOStartCallBuilder.create(loc, rewriter, {kernelId, totalAlternatives});
                rewriter.setInsertionPoint(ifOp.getThenRegion().front().getTerminator());
                rtPGOEndCallBuilder.create(loc, rewriter, {kernelId, totalAlternatives});
            ...
            for (int i = 0; i < numAlternatives; i++) {
                if (timings[i].size() == 0) {
                    ...
                    // or median
                    avgs.push_back(std::accumulate(timings[i].begin(), timings[i].end(), 0.0f) / timings[i].size());
                    llvm::errs() << "Alternative " << i << "," << descs[i] << " is " << avgs[i] << "\n";
                }
            }

            int bestAlt = std::distance(avgs.begin(), std::min_element(avgs.begin(), avgs.end()));
    ...
    LowerGPUAlternativesOp(MLIRContext *context, LLVMTypeConverter &typeConverter, StringRef gpuBinaryAnnotation,
                           StringRef gpuTarget)
        : OpRewritePattern<polygeist::AlternativesOp>(context), GpuRuntimeCallBuilders(context, typeConverter),
...
// return %array
Value ConvertLaunchFuncOpToGpuRuntimeCallPattern::generateParamsArray(gpu::LaunchFuncOp launchOp, OpAdaptor adaptor,
                                                                      OpBuilder &builder) const {
    auto loc = launchOp.getLoc();
    auto numKernelOperands = launchOp.getNumKernelOperands();
    SmallVector<Value, 4> arguments = adaptor.getOperands().take_back(numKernelOperands);
    auto numArguments = arguments.size();
    SmallVector<Type, 4> argumentTypes;
    argumentTypes.reserve(numArguments);
    for (auto argument : arguments)
        argumentTypes.push_back(argument.getType());
    auto structType = LLVM::LLVMStructType::getNewIdentified(context, StringRef(), argumentTypes);
    auto one = builder.create<LLVM::ConstantOp>(loc, llvmInt32Type, 1);
    auto structPtr =
        builder.create<LLVM::AllocaOp>(loc, LLVM::LLVMPointerType::get(builder.getContext()), structType, one,
                                       /*alignment=*/0);
    auto arraySize = builder.create<LLVM::ConstantOp>(loc, llvmInt32Type, numArguments);
    auto arrayPtr =
        builder.create<LLVM::AllocaOp>(loc, llvmPointerPointerType, llvmPointerType, arraySize, /*alignment=*/0);
    for (const auto &en : llvm::enumerate(arguments)) {
        auto fieldPtr = builder.create<LLVM::GEPOp>(loc, LLVM::LLVMPointerType::get(builder.getContext()), structType,
                                                    structPtr, ArrayRef<LLVM::GEPArg>{0, en.index()});
        builder.create<LLVM::StoreOp>(loc, en.value(), fieldPtr);
        auto elementPtr = builder.create<LLVM::GEPOp>(loc, llvmPointerType, llvmPointerPointerType, arrayPtr,
                                                      ArrayRef<LLVM::GEPArg>{en.index()});
        auto casted = builder.create<LLVM::BitcastOp>(loc, llvmPointerType, fieldPtr);
...
// }
Value ConvertLaunchFuncOpToGpuRuntimeCallPattern::generateKernelNameConstant(StringRef moduleName, StringRef name,
                                                                             Location loc, OpBuilder &builder) const {
    // Make sure the trailing zero is included in the constant.
    std::vector<char> kernelName(name.begin(), name.end());
    kernelName.push_back('\0');

    std::string globalName = std::string(llvm::formatv("{0}_{1}_kernel_name", moduleName, name));
    return LLVM::createGlobalString(loc, builder, globalName, StringRef(kernelName.data(), kernelName.size()),
...
static LogicalResult areAllLLVMTypes(Operation *op, ValueRange operands, ConversionPatternRewriter &rewriter) {
    if (!llvm::all_of(operands, [](Value value) { return LLVM::isCompatibleType(value.getType()); }))
...
LogicalResult ConvertLaunchFuncOpToGpuRuntimeCallPattern::matchAndRewrite(gpu::LaunchFuncOp launchOp, OpAdaptor adaptor,
                                                                          ConversionPatternRewriter &rewriter) const {
    if (failed(areAllLLVMTypes(launchOp, adaptor.getOperands(), rewriter)))
        return failure();

    if (launchOp.getAsyncDependencies().size() > 1)
        return rewriter.notifyMatchFailure(launchOp, "Cannot convert with more than one async dependency.");
    ...
    if (!launchOp.getAsyncToken() && !launchOp.getAsyncDependencies().empty())
        return rewriter.notifyMatchFailure(launchOp, "Cannot convert non-async op with async dependencies.");
    ...
    auto kernelModule =
        SymbolTable::lookupNearestSymbolFrom<gpu::GPUModuleOp>(launchOp, launchOp.getKernelModuleName());
    assert(kernelModule && "expected a kernel module");

    auto getFuncStubName = [](StringRef moduleName, StringRef name) {
        return std::string(llvm::formatv("__polygeist_{0}_{1}_device_stub", moduleName, name));
    };
    auto getFuncGlobalName = [](StringRef moduleName, StringRef name) {
        return std::string(llvm::formatv("__polygeist_{0}_{1}_fun_ptr", moduleName, name));
    };
    ...
    ModuleOp moduleOp = launchOp->getParentOfType<ModuleOp>();
    {
        ...
        LLVM::LLVMFuncOp ctor =
            dyn_cast_or_null<LLVM::LLVMFuncOp>(SymbolTable::lookupSymbolIn(moduleOp, ctorNameBuffer));
        SmallString<128> dtorNameBuffer(kernelModule.getName());
        dtorNameBuffer.append(kGpuModuleDtorSuffix);
        LLVM::LLVMFuncOp dtor =
            dyn_cast_or_null<LLVM::LLVMFuncOp>(SymbolTable::lookupSymbolIn(moduleOp, dtorNameBuffer));
        if (!ctor) {
            assert(!dtor && "gpu module constructor does not exist but destructor does");
            ctor = moduleBuilder.create<LLVM::LLVMFuncOp>(
                loc, ctorNameBuffer, LLVM::LLVMFunctionType::get(LLVM::LLVMVoidType::get(moduleOp.getContext()), {}),
                LLVM::Linkage::Private);
            dtor = moduleBuilder.create<LLVM::LLVMFuncOp>(
                loc, dtorNameBuffer, LLVM::LLVMFunctionType::get(LLVM::LLVMVoidType::get(moduleOp.getContext()), {}),
                LLVM::Linkage::Private);

            auto binaryAttr = kernelModule->getAttrOfType<StringAttr>(gpuBinaryAnnotation);
            if (!binaryAttr) {
                kernelModule.emitOpError() << "missing " << gpuBinaryAnnotation << " attribute";
                return failure();
            }
            ...
            auto fatBinWrapper = moduleBuilder.create<LLVM::GlobalOp>(
                ...
                std::string(llvm::formatv("__polygeist_{0}_fatbin_wrapper", moduleName)),
                /* initValue */ mlir::Attribute(),
                /* alignment */ 8, /* addrSpace */ 0);
            fatBinWrapper.setSectionAttr(moduleBuilder.getStringAttr(fatbinSectionName));
            ...
            auto fatbinMagicVal = globalBuilder.create<LLVM::ConstantOp>(loc, llvmInt32Type, fatMagic);
            ...
            Value constructedStruct = globalBuilder.create<LLVM::UndefOp>(loc, fatBinWrapperType);
            {
                ...
                constructedStruct = globalBuilder.create<LLVM::InsertValueOp>(
                    loc, fatBinWrapperType, constructedStruct, fatbinMagicVal, globalBuilder.getDenseI64ArrayAttr(i++));
                constructedStruct = globalBuilder.create<LLVM::InsertValueOp>(loc, fatBinWrapperType, constructedStruct,
                                                                              fatbinVersionVal,
                ...
                Value data = LLVM::createGlobalString(loc, globalBuilder, nameBuffer.str(), binaryAttr.getValue(),
                                                      LLVM::Linkage::Internal, /*opaquePointers*/ true);
                constructedStruct = globalBuilder.create<LLVM::InsertValueOp>(
                    loc, fatBinWrapperType, constructedStruct, data, globalBuilder.getDenseI64ArrayAttr(i++));
                constructedStruct = globalBuilder.create<LLVM::InsertValueOp>(
                    loc, fatBinWrapperType, constructedStruct, nullPtr, globalBuilder.getDenseI64ArrayAttr(i++));
            }
            ...
            auto bitcastOfWrapper = ctorBuilder.create<LLVM::BitcastOp>(loc, llvmPointerType, addressOfWrapper);
            auto module = rtRegisterFatBinaryCallBuilder.create(loc, ctorBuilder, {bitcastOfWrapper});
            ...
            auto moduleGlobal = moduleBuilder.create<LLVM::GlobalOp>(
                loc, llvmPointerPointerType, /* isConstant */ false, LLVM::Linkage::Internal, moduleGlobalName,
                /* initValue */ mlir::Attribute(),
                /* alignment */ 8, /* addrSpace */ 0);
            ...
            for (Operation &op : kernelModule->getRegion(0).front()) {
                if (LLVM::LLVMFuncOp f = dyn_cast<LLVM::LLVMFuncOp>(op)) {
                    ...
                    if (symbolUses && symbolUses->empty())
                        continue;
                    auto kernelName = generateKernelNameConstant(launchOp.getKernelModuleName().getValue(), f.getName(),
                                                                 loc, ctorBuilder);
                    ...
                    // cudaLaunchKernel
                    auto stub = moduleBuilder.create<LLVM::LLVMFuncOp>(loc, getFuncStubName(moduleName, f.getName()),
                    ...
                    auto bitcast = ctorBuilder.create<LLVM::BitcastOp>(loc, llvmPointerType, aoo);
                    auto ret = rtRegisterFunctionCallBuilder.create(
                        ...
                        {module.getResult(), bitcast, kernelName, kernelName,
                         ...
                         ctorBuilder.create<LLVM::ConstantOp>(loc, llvmInt32Type, -1), nullPtr, nullPtr, nullPtr,
                         nullPtr, nullPtr});
                    ...
                    auto symbolName = [&]() {
                        ...
                        std::string globalName =
                            std::string(llvm::formatv("__polygeist_{0}_{1}_global_name", moduleName, name));

                        return LLVM::createGlobalString(loc, ctorBuilder, globalName,
                    ...
                    auto aoo = ctorBuilder.create<LLVM::AddressOfOp>(loc, stub);
                    auto bitcast = ctorBuilder.create<LLVM::BitcastOp>(loc, llvmPointerType, aoo);
                    auto globalTy = dyn_cast<LLVM::LLVMPointerType>(aoo.getType()).getElementType();
            ...
            if (gpuTarget == "cuda")
                rtRegisterFatBinaryEndCallBuilder.create(loc, ctorBuilder, {module.getResult()});
            ctorBuilder.create<LLVM::ReturnOp>(loc, ValueRange());
            auto ctorSymbol = FlatSymbolRefAttr::get(ctor);
            moduleBuilder.create<LLVM::GlobalCtorsOp>(loc, moduleBuilder.getArrayAttr({std::move(ctorSymbol)}),
                                                      moduleBuilder.getI32ArrayAttr({65535}));
            {
                ...
                rtUnregisterFatBinaryCallBuilder.create(loc, dtorBuilder, module.getResult());
                dtorBuilder.create<LLVM::ReturnOp>(loc, ValueRange());
                auto dtorSymbol = FlatSymbolRefAttr::get(dtor);
                moduleBuilder.create<LLVM::GlobalDtorsOp>(loc, moduleBuilder.getArrayAttr({std::move(dtorSymbol)}),
                                                          moduleBuilder.getI32ArrayAttr({65535}));
            }
        }
    }
    ...
    std::string funcStubName =
        getFuncStubName(launchOp.getKernelModuleName().getValue(), launchOp.getKernelName().getValue());

    auto stub = dyn_cast_or_null<LLVM::LLVMFuncOp>(SymbolTable::lookupSymbolIn(moduleOp, funcStubName));
    assert(!!stub);
    ...
    Value stream = adaptor.getAsyncDependencies().empty() ? nullpointer : adaptor.getAsyncDependencies().front();
    // Create array of pointers to kernel arguments.
    auto kernelParams = generateParamsArray(launchOp, adaptor, rewriter);
    auto nullpointerpointer = rewriter.create<LLVM::ZeroOp>(loc, llvmPointerPointerType);
    Value dynamicSharedMemorySize =
        launchOp.getDynamicSharedMemorySize() ? launchOp.getDynamicSharedMemorySize() : zero;

    if (mgpuDebugLaunches || moduleOp->hasAttr("polygeist.mgpu_debug_launches")) {
        LLVM::LLVMFuncOp printfFunc = moduleOp.lookupSymbol<LLVM::LLVMFuncOp>("printf");
        if (!printfFunc) {
            OpBuilder moduleBuilder(moduleOp.getBodyRegion());
            auto i32Ty = IntegerType::get(moduleOp.getContext(), 32);
            auto ptrTy = LLVM::LLVMPointerType::get(moduleOp.getContext());
            auto printfTy = LLVM::LLVMFunctionType::get(i32Ty, ArrayRef<Type>{ptrTy}, /*isVarArg=*/true);
            printfFunc = moduleBuilder.create<LLVM::LLVMFuncOp>(loc, "printf", printfTy);
        }

        static int64_t dbgMsgId = 0;
        int64_t launchNum = ++dbgMsgId;
        auto llvmI64Type = IntegerType::get(moduleOp.getContext(), 64);
        Value launchNumVal = rewriter.create<LLVM::ConstantOp>(loc, llvmI64Type, rewriter.getI64IntegerAttr(launchNum));

        std::string kernelNameStr =
            (launchOp.getKernelModuleName().getValue() + "::" + launchOp.getKernelName().getValue()).str();
        kernelNameStr.push_back('\0');
        std::string msgGlobalName = llvm::formatv("__polygeist_mgpu_launch_msg_{0}", launchNum - 1).str();
        Value msgPtr = LLVM::createGlobalString(loc, rewriter, msgGlobalName, kernelNameStr, LLVM::Linkage::Internal,
                                                /*opaquePointers=*/true);

        auto deviceAttr = launchOp->getAttrOfType<IntegerAttr>("polygeist.mgpu.device_index");
        auto offsetAttr = launchOp->getAttrOfType<IntegerAttr>("polygeist.mgpu.linear_offset");
        auto countAttr = launchOp->getAttrOfType<IntegerAttr>("polygeist.mgpu.linear_count");
        ...
        if (hasRange) {
            ...
            printfArgs.push_back(rewriter.create<LLVM::ConstantOp>(loc, llvmInt32Type, deviceAttr));
            printfArgs.push_back(
                rewriter.create<LLVM::ConstantOp>(loc, llvmI64Type, rewriter.getI64IntegerAttr(offset)));
            printfArgs.push_back(rewriter.create<LLVM::ConstantOp>(loc, llvmI64Type, rewriter.getI64IntegerAttr(end)));
        } else {
        ...
        fmt.push_back('\0');
        std::string fmtGlobalName = llvm::formatv("__polygeist_mgpu_launch_fmt_{0}", launchNum - 1).str();
        Value fmtPtr = LLVM::createGlobalString(loc, rewriter, fmtGlobalName, fmt, LLVM::Linkage::Internal,
                                                /*opaquePointers=*/true);
        printfArgs.insert(printfArgs.begin(), fmtPtr);
        rewriter.create<LLVM::CallOp>(loc, printfFunc, printfArgs);
    }

    auto deviceIndexAttr = launchOp->getAttrOfType<IntegerAttr>("polygeist.mgpu.device_index");
    if (deviceIndexAttr) {
        LLVM::LLVMFuncOp setDeviceFunc = moduleOp.lookupSymbol<LLVM::LLVMFuncOp>("mgpurtSetDevice");
        if (!setDeviceFunc) {
            OpBuilder moduleBuilder(moduleOp.getBodyRegion());
            auto i32Ty = IntegerType::get(moduleOp.getContext(), 32);
            setDeviceFunc = moduleBuilder.create<LLVM::LLVMFuncOp>(
                loc, "mgpurtSetDevice", LLVM::LLVMFunctionType::get(i32Ty, ArrayRef<Type>{i32Ty}));
        }
        Value deviceArg = rewriter.create<LLVM::ConstantOp>(loc, llvmInt32Type, deviceIndexAttr);
        rewriter.create<LLVM::CallOp>(loc, setDeviceFunc, deviceArg);
    }

    auto launchCall =
        rtLaunchKernelErrCallBuilder.create(loc, rewriter,
    ...
    if (errOp) {
        auto cast = rewriter.create<arith::IndexCastOp>(loc, rewriter.getIndexType(), launchCall->getResult(0));
...
struct ReplaceErrOpWithSuccess : public OpRewritePattern<polygeist::GPUErrorOp> {
    using OpRewritePattern<polygeist::GPUErrorOp>::OpRewritePattern;
    const char *PATTERN = "lower-gpu-alternatives";

    LogicalResult matchAndRewrite(polygeist::GPUErrorOp errOp, PatternRewriter &rewriter) const override {
        rewriter.setInsertionPoint(errOp);
        rewriter.eraseOp(errOp.getBody()->getTerminator());
        rewriter.inlineBlockBefore(errOp.getBody(), errOp);
        rewriter.setInsertionPoint(errOp);
...
struct GPUFuncOpLowering : public ConvertOpToLLVMPattern<gpu::GPUFuncOp> {
  ...
  public:
    ...
    GPUFuncOpLowering(LLVMTypeConverter &converter, unsigned allocaAddrSpace, StringAttr kernelAttributeName)
        : ConvertOpToLLVMPattern<gpu::GPUFuncOp>(converter), allocaAddrSpace(allocaAddrSpace),
          kernelAttributeName(kernelAttributeName) {}

    LogicalResult matchAndRewrite(gpu::GPUFuncOp gpuFuncOp, OpAdaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const override {
        ...
        for (const auto &en : llvm::enumerate(gpuFuncOp.getWorkgroupAttributions())) {
            ...
            auto elementType = typeConverter->convertType(type.getElementType()).template cast<Type>();
            auto arrayType = LLVM::LLVMArrayType::get(elementType, numElements);
            std::string name = std::string(llvm::formatv("__wg_{0}_{1}", gpuFuncOp.getName(), en.index()));
            auto globalOp = rewriter.create<LLVM::GlobalOp>(
                gpuFuncOp.getLoc(), arrayType, /*isConstant=*/false, LLVM::Linkage::Internal, name,
                /*value=*/Attribute(),
        ...
        if (!typePair)
            return rewriter.notifyMatchFailure(gpuFuncOp->getLoc(), "failed to convert signature");
        ...
        auto llvmFuncOp =
            rewriter.create<LLVM::LLVMFuncOp>(gpuFuncOp.getLoc(), gpuFuncOp.getName(), funcType,
...

        {
            ...
            for (const auto &en : llvm::enumerate(workgroupBuffers)) {
                ...
                auto descr = MemRefDescriptor::fromStaticShape(rewriter, loc, *getTypeConverter(), type, memory);
            ...
            for (const auto &en : llvm::enumerate(gpuFuncOp.getPrivateAttributions())) {
                ...
                auto ptrType = LLVM::LLVMPointerType::get(
                    typeConverter->convertType(type.getElementType()).template cast<Type>(), allocaAddrSpace);
                Value numElements =
                    rewriter.create<LLVM::ConstantOp>(gpuFuncOp.getLoc(), int64Ty, type.getNumElements());
                Value allocated =
                    rewriter.create<LLVM::AllocaOp>(gpuFuncOp.getLoc(), ptrType, numElements, /*alignment=*/0);
                auto descr = MemRefDescriptor::fromStaticShape(rewriter, loc, *getTypeConverter(), type, allocated);
                signatureConversion.remapInput(numProperArguments + numWorkgroupAttributions + en.index(), descr);
            }
        }

        // Move the region to the new function, update the entry block signature.
        rewriter.inlineRegionBefore(gpuFuncOp.getBody(), llvmFuncOp.getBody(), llvmFuncOp.end());
        if (failed(rewriter.convertRegionTypes(&llvmFuncOp.getBody(), *typeConverter, &signatureConversion)))
            return rewriter.notifyMatchFailure(gpuFuncOp->getLoc(), "failed to apply signature conversion");
        rewriter.eraseOp(gpuFuncOp);
        return success();
    }
};

static LogicalResult isAsyncWithZeroDependencies(ConversionPatternRewriter &rewriter, gpu::AsyncOpInterface op) {
...
class ConvertAllocOpToGpuRuntimeCallPattern : public ConvertOpToGpuRuntimeCallPattern<gpu::AllocOp> {
  public:
  ...
  private:
    LogicalResult matchAndRewrite(gpu::AllocOp allocOp, OpAdaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const override {
        if (adaptor.getHostShared())
            return rewriter.notifyMatchFailure(allocOp, "host_shared allocation is not supported");
        ...
        Value allocatedPtr = rtMemAllocCallBuilder.create(loc, rewriter, {sizeBytes, stream}).getResult();
        allocatedPtr = rewriter.create<LLVM::BitcastOp>(loc, elementPtrType, allocatedPtr);
...
struct FuncOpLowering : public ConvertOpToLLVMPattern<func::FuncOp> {
  public:
    ...
    LogicalResult matchAndRewrite(func::FuncOp funcOp, OpAdaptor adaptor,
        ...
        if (!typePair)
            return rewriter.notifyMatchFailure(funcOp->getLoc(), "failed to convert signature");

        auto [convertedType, conversionSignature] = *typePair;
        SmallVector<NamedAttribute> attributes =
            convertFuncAttributes(funcOp, *typeConverter, conversionSignature, rewriter);

        LLVM::Linkage linkage = LLVM::Linkage::External;
        if (funcOp->hasAttr(kLLVMLinkageAttrName)) {
            auto attr = funcOp->getAttr(kLLVMLinkageAttrName).cast<mlir::LLVM::LinkageAttr>();
            linkage = attr.getLinkage();
        }
        auto newFuncOp = rewriter.create<LLVM::LLVMFuncOp>(funcOp.getLoc(), funcOp.getName(), convertedType, linkage,
                                                           /*dsoLocal=*/false, /*cconv=*/LLVM::CConv::C,
                                                           /*comdat=*/nullptr, attributes);
        rewriter.inlineRegionBefore(funcOp.getBody(), newFuncOp.getBody(), newFuncOp.end());
        if (failed(rewriter.convertRegionTypes(&newFuncOp.getBody(), *typeConverter, &conversionSignature))) {
...
struct CallOpLowering : public ConvertOpToLLVMPattern<func::CallOp> {
  public:
    ...
    LogicalResult matchAndRewrite(func::CallOp callOp, OpAdaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const override {
        unsigned numResults = callOp.getNumResults();
        SmallVector<Type, 1> callResultTypes;
        if (!callOp.getResults().empty()) {
            callResultTypes.push_back(convertAndPackFunctionResultType(callOp.getCalleeType(), *typeConverter));
            if (!callResultTypes.back()) {
                return rewriter.notifyMatchFailure(callOp.getLoc(), "failed to convert callee signature");
            }
        }

        auto newCallOp =
            rewriter.create<LLVM::CallOp>(callOp->getLoc(), callResultTypes, adaptor.getOperands(), callOp->getAttrs());
        ...
        for (auto index : llvm::seq<unsigned>(0, numResults)) {
            results.push_back(rewriter.create<LLVM::ExtractValueOp>(callOp->getLoc(), newCallOp->getResult(0), index));
        }
  ...
  public:
    ...
    LogicalResult matchAndRewrite(func::ReturnOp returnOp, OpAdaptor adaptor,
        ...
        if (returnOp->getNumOperands() <= 1) {
            rewriter.replaceOpWithNewOp<LLVM::ReturnOp>(returnOp, adaptor.getOperands());
            return success();
        }

        auto returnedType =
            LLVM::LLVMStructType::getLiteral(returnOp->getContext(), llvm::to_vector(adaptor.getOperands().getTypes()));
        Value packed = rewriter.create<LLVM::UndefOp>(returnOp->getLoc(), returnedType);
        for (const auto &[index, value] : llvm::enumerate(adaptor.getOperands())) {
            packed = rewriter.create<LLVM::InsertValueOp>(returnOp->getLoc(), packed, value, index);
        }
...
struct ReconcileUnrealizedPointerCasts : public OpRewritePattern<UnrealizedConversionCastOp> {
    using OpRewritePattern<UnrealizedConversionCastOp>::OpRewritePattern;
    LogicalResult matchAndRewrite(UnrealizedConversionCastOp ucc, PatternRewriter &rewriter) const override {
        ...
        if (!(inputTy.isa<LLVM::LLVMPointerType>() && outputTy.isa<LLVM::LLVMPointerType>()))
            return failure();
...
static void populateCStyleMemRefLoweringPatterns(RewritePatternSet &patterns, LLVMTypeConverter &typeConverter) {
    patterns.add<CAllocaOpLowering, CAllocOpLowering, CDeallocOpLowering, GetGlobalOpLowering, GlobalOpLowering,
                 CLoadOpLowering, CStoreOpLowering, AllocaScopeOpLowering, CAtomicRMWOpLowering>(typeConverter);
}

/// Appends the patterns lowering operations from the Func dialect to the LLVM
/// dialect using the C-style type conversion, i.e. converting memrefs to
/// pointer to arrays of arrays.
static void populateCStyleGPUFuncLoweringPatterns(RewritePatternSet &patterns, LLVMTypeConverter &typeConverter,
                                                  std::string gpuTarget) {
    patterns.add<GPUFuncOpLowering>(
        typeConverter,
        /*allocaAddrSpace=*/0,
        StringAttr::get(&typeConverter.getContext(), gpuTarget == "cuda"
...
/// dialect using the C-style type conversion, i.e. converting memrefs to
/// pointer to arrays of arrays.
static void populateCStyleFuncLoweringPatterns(RewritePatternSet &patterns, LLVMTypeConverter &typeConverter) {
...
struct ConvertPolygeistToLLVMPass : public ConvertPolygeistToLLVMBase<ConvertPolygeistToLLVMPass> {
    ...
    ConvertPolygeistToLLVMPass(bool useBarePtrCallConv, unsigned indexBitwidth, bool useAlignedAlloc,
                               const llvm::DataLayout &dataLayout, bool useCStyleMemRef, bool onlyGpuModules,
                               std::string gpuTarget, bool mgpuDebugLaunches = false) {
    ...
    void convertModule(ModuleOp m, bool gpuModule) {
        ...
        if (gpuModule) {
            ...
            for (auto func : m.getOps<func::FuncOp>()) {
                func->setAttr(LLVM::LLVMDialect::getEmitCWrapperAttrName(), UnitAttr::get(&getContext()));
            }
        }
        ...
        if (useCStyleMemRef) {
            converter.addConversion([&](MemRefType type) -> std::optional<Type> {
                ...
                if (!elTy)
                    return Type();
                return LLVM::LLVMPointerType::get(type.getContext(), type.getMemorySpaceAsInt());
            });
        }
...

        {
            ...
            patterns.add<LowerGPUAlternativesOp>(&getContext(), converter, gpu::getDefaultGpuBinaryAnnotation(),
                                                 gpuTarget);
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (gpuModule) {
                if (gpuTarget == "cuda") {
                    ...
                    populateGpuToROCDLConversionPatterns(converter, patterns, gpu::amd::Runtime::HIP);
                }
            }
            ...
            if (useCStyleMemRef) {
                patterns.add<ConvertLaunchFuncOpToGpuRuntimeCallPattern>(
                    converter, gpu::getDefaultGpuBinaryAnnotation(), gpuTarget, mgpuDebugLaunches);
                patterns.add<ConvertAllocOpToGpuRuntimeCallPattern>(converter);
            }

            patterns.add<LLVMOpLowering, GlobalOpTypeConversion, ReturnOpTypeConversion, GetFuncOpConversion>(
                converter);
            ...
            auto areAllTypesConverted = [&](Operation *op) -> std::optional<bool> {
                ...
                return convertedResultTypes == op->getResultTypes() && convertedOperandTypes == op->getOperandTypes();
            };
            ...
            if (gpuModule) {
                ...
                target.addIllegalOp<LLVM::CosOp, LLVM::ExpOp, LLVM::Exp2Op, LLVM::FAbsOp, LLVM::FCeilOp, LLVM::FFloorOp,
                                    LLVM::LogOp, LLVM::Log10Op, LLVM::Log2Op, LLVM::PowOp, LLVM::SinOp, LLVM::SqrtOp>();
                target.addLegalOp<gpu::YieldOp, gpu::GPUModuleOp, gpu::ModuleEndOp>();
            }
            target.addDynamicallyLegalOp<omp::ParallelOp, omp::WsLoopOp>(
                [&](Operation *op) { return converter.isLegal(&op->getRegion(0)); });
            target.addIllegalOp<scf::ForOp, scf::IfOp, scf::ParallelOp, scf::WhileOp, scf::ExecuteRegionOp,
                                func::FuncOp>();
            target.addLegalOp<omp::TerminatorOp, omp::TaskyieldOp, omp::FlushOp, omp::YieldOp, omp::BarrierOp,
                              omp::TaskwaitOp>();
            target.addDynamicallyLegalDialect<LLVM::LLVMDialect>(areAllTypesConverted);
            target.addDynamicallyLegalOp<LLVM::GlobalOp>([&](LLVM::GlobalOp op) -> std::optional<bool> {
                if (converter.convertType(op.getGlobalType()) == op.getGlobalType())
                    return true;
                return std::nullopt;
            });
            target.addDynamicallyLegalOp<LLVM::ReturnOp>([&](LLVM::ReturnOp op) -> std::optional<bool> {
            ...
            if (i == 0) {
                ...
                target.addDynamicallyLegalOp<async::ExecuteOp>(
                    [&](async::ExecuteOp eo) { return eo->hasAttr("polygeist.handled"); });
            } else if (i == 1) {
    ...
    void runOnOperation() override {
        ...
        for (auto gpum : gpuModules) {
            ...
            auto triple =
                m->getAttr(StringRef("polygeist.gpu_module." + LLVM::LLVMDialect::getTargetTripleAttrName().str()));
            ...
            tmpModule->setAttr(("dlti." + DataLayoutSpecAttr::kAttrKeyword).str(),
...
std::unique_ptr<Pass> mlir::polygeist::createConvertPolygeistToLLVMPass(const LowerToLLVMOptions &options,
                                                                        ...
                                                                        std::string gpuTarget, bool mgpuDebugLaunches) {
    ...
    bool useAlignedAlloc = (allocLowering == LowerToLLVMOptions::AllocLowering::AlignedAlloc);
    return std::make_unique<ConvertPolygeistToLLVMPass>(options.useBarePtrCallConv, options.getIndexBitwidth(),
                                                        useAlignedAlloc, options.dataLayout, useCStyleMemRef,
                                                        onlyGpuModules, gpuTarget, mgpuDebugLaunches);
}
