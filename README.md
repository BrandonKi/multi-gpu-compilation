# Multi-GPU Compilation Dialect

Standalone dialect built to integrate with Polygeist transformations.

## Build

Download cuda.

First build Polygeist and llvm/mlir by following the instructions below. Enable cuda support.
...
~/Polygeist/build/bin/polygeist-opt ~/Polygeist/test/polygeist-opt/multigpu-to-gpu.mlir -identity-pass -multigpu-to-cuda -canonicalize --mlir-print-ir-after-all &> log.txt

# can optionally use pass visualization
python3 generate_viewer.py
```

Generate Dialect Docs

```bash
mlir-tblgen -gen-dialect-doc MultiGpuOps.td -I /home/kirincichb/Polygeist/llvm-project/mlir/include -o MultiGpuOps.md | pandoc MultiGpuOps.md -f markdown -t html --metadata title="MultiGPU Dialect" -s --self-contained --toc --toc-depth=3 -c https://cdn.jsdelivr.net/npm/sakura.css/cs
s/sakura.css -c /home/kirincichb/Polygeist/custom.css -o MultiGpuOps.html
```

# Polygeist Build instructions
